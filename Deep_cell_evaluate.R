...
f.path <- "../DeepProfiler/InceptionResnetV2-mean_profiles.csv"
data <- read.csv(f.path, stringsAsFactors = FALSE)
...
#variables <- c(common_features, metadata_data)  #for TA-ORF dataset
...
dmeta <- df %>% select(one_of(metadata_df)) %>% dplyr::collect()
...
#Whitening of data
population_whitened <- cytominer::whiten(
  population = df,
...
variables <- colnames(population_whitened)
metadata <- stringr::str_subset(variables, "^Meta")
variables <- setdiff(variables, metadata)


profiles.nrm <- population_whitened %>% select(one_of(c(metadata, variables))) %>% dplyr::collect()
...
feats  <- c(
  stringr::str_subset(colnames(prf), "^PC")
)
# feats  <- c(
...
evaluate.moa <- function(cr, profiles.meta, quant = 0.95, type.eval = "global", k = 1, skip.comp = F) {
  ...
  saveRDS(cr.melt, paste0("dp_cr_",  profile.type, ifelse(profile.type == "mix", paste0(mix1, mix2), ""), ".rds"))
